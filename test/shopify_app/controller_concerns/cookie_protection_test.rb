...
class CookieProtectionController < ActionController::Base
  include ShopifyApp::CookieProtection
...
class CookieProtectionControllerTest < ActionController::TestCase
  tests CookieProtectionController
  ...
  test '#fullpage_redirect_to raises an exception when no Shopify domains are available' do
    with_application_test_routes do
      ...
      assert_raise ShopifyApp::CookieProtection::ShopifyDomainNotFound do
        get :redirect
      end
    end
  end
  ...
  def with_application_test_routes
    with_routing do |set|
      set.draw do
        get '/' => 'cookie_protection#index'
        get '/second_login' => 'cookie_protection#second_login'
        get '/redirect' => 'cookie_protection#redirect'
        get '/raise_unauthorized' => 'cookie_protection#raise_unauthorized'
      end
      yield
    end
  end
