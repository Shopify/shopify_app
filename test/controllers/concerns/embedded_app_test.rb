...
class EmbeddedAppTest < ActionDispatch::IntegrationTest
  ...
  test "content security policy only sets frame-ancestors by default" do
    ShopifyApp.configuration.embedded_app = true
    ...
    if csp_header
      assert_includes csp_header, "frame-ancestors"
      assert_not_includes csp_header, "script-src"
    end
  end
end
