(function () {
  ...
  StorageAccessHelper.prototype.setNormalizedLink = function (storageAccessStatus) {
    return storageAccessStatus === ACCESS_GRANTED_STATUS
      ? this.redirectData.hasStorageAccessUrl
      : this.redirectData.doesNotHaveStorageAccessUrl;
  };
  ...
  StorageAccessHelper.prototype.redirectToAppTLD = function (storageAccessStatus) {
    ...
    window.parent.postMessage(data, this.redirectData.host);
  };

  StorageAccessHelper.prototype.redirectToAppsIndex = function () {
    window.parent.location.href = this.redirectData.host + '/apps';
  };
  ...
  StorageAccessHelper.prototype.sameSiteNoneIncompatible = function (ua) {
    return (
      ua.includes('iPhone OS 12_') ||
      ua.includes('iPad; CPU OS 12_') || //iOS 12
      (ua.includes('UCBrowser/')
        ? this.isOlderUcBrowser(ua) //UC Browser < 12.13.2
        : ua.includes('Chrome/5') || ua.includes('Chrome/6')) ||
      ua.includes('Chromium/5') ||
      ua.includes('Chromium/6') ||
      (ua.includes(' OS X 10_14_') &&
        ((ua.includes('Version/') && ua.includes('Safari')) || //Safari on MacOS 10.14
          ua.endsWith('(KHTML, like Gecko)')))
    ); //Web view on MacOS 10.14
  };
  ...
  StorageAccessHelper.prototype.handleRequestStorageAccess = function () {
    return document
      ...
      .then(this.grantedStorageAccess.bind(this), this.redirectToAppsIndex.bind(this, ACCESS_DENIED_STATUS));
  };
  ...
  StorageAccessHelper.prototype.manageStorageAccess = function () {
    return document.hasStorageAccess().then(
      function (hasAccess) {
        if (hasAccess) {
  ...
  StorageAccessHelper.prototype.setUpHelper = function () {
    ...
    return new ITPHelper({
      redirectUrl: shopifyData.host + '/apps/' + shopifyData.apiKey + shopifyData.returnTo,
    });
  };
