...
Version 18.1.2 updates to use App Bridge 2.0 redirect with `host` param when attempting to break out of an iframe. This happens when an app is installed, requires new access scopes, or re-authentication because the login session is expired.
