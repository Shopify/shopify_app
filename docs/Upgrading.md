...
If you need upgrade by more than one major version (e.g. from v18 to v20), we recommend doing so one by major version at a time. Deploy each into production to help to detect problems earlier.
...
To investigate the cause, you can add a breakpoint to the `rescue` clause of `ShopifyApp::CallbackController`.
...
The `shopify_app` initializer must configure the `ShopifyAPI::Context`. The Rails generator will generate a block in the `shopify_app` initializer. To do so manually, you can refer to `after_initialize` block in the [template](/lib/generators/shopify_app/install/templates/shopify_app.rb.tt).
