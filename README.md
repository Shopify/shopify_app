...
If you want to use the application.js file as generated by the Rails asset pipeline, you can do so by moving the `ShopifyApp.configure`
to `config/application.rb` like so:

```ruby
config.after_initialize do
	ShopifyApp.configure do |shopify_config|
		shopify_config.scripttags = [
			{event: 'onload', src: "https://example.com#{ActionController::Base.helpers.javascript_url('application')}"}
		]
	end
end
```

This will ensure the configuration is done after the asset file is fingerprinted.
